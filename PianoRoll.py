...
class MidiFile(mido.MidiFile):
    ...
    @staticmethod
    def to_mid(pianoroll):
        if len(pianoroll.shape) >= 3:
            print(" --> wrong pianoroll dimensions")
            pianoroll = pianoroll[0]
            # only pianorolls with one channel!

        p = np.asarray(pianoroll)
        p = np.swapaxes(p, 0, 1)
        ...
        for notes in p:
            ...
            for note in range(len(notes)):
                if notes[note] != 0 and notes[note] not in last_notes:
                    track.append(mido.Message('note_on', note=note, velocity=127, time=0, channel=0))
                    #track.append(mido.Message('note_off', note=note, velocity=65, time=0, channel=0))

            #print(p[100])

            for last_note in last_notes:
                if last_note not in notes:
                    track.append(mido.Message('note_on', note=last_note, velocity=0, time=0, channel=0))

            track.append(mido.Message('program_change', program=0, channel=0, time=time))

            last_notes = notes

        new_mid.save('output/output.mid')

        #for msg in mido.MidiFile('output/output.mid'):
            #print(msg)
